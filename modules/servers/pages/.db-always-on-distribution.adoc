...
image::dev@platform:admin:sql-server-config-manager.png[Настройки протоколов в конфигурации {mssql} Server]
+
...
image::dev@platform:admin:sql-server-config-manager-properties.png[Настройки службы SQL Server]
+
...
image::dev@platform:admin:always-on-enable.png[Настройка поддержки AlwaysOn в конфигурации службы {mssql} Server]
+
...
image::dev@platform:admin:always-on-create.png[Команда создания группы доступности]
+
Будет запущен мастер.
+
.Мастер создания группы доступности
image::dev@platform:admin:always-on-start.png[Мастер создания группы доступности]
+
. На странице _Specify Options_ укажите название группы. Например, `{dv}DBs`. Другие параметры изменять не требуется.
+
.Мастер создания группы доступности
image::dev@platform:admin:always-on-name.png[Мастер создания группы доступности]
+
. На странице _Select Databases_ выберите БД {dv}, которые нужно включить в группу доступности.
+
.Мастер создания группы доступности
image::dev@platform:admin:always-on-next.png[Мастер создания группы доступности]
+
****
...
+
.Мастер создания группы доступности
image::dev@platform:admin:always-on-replicas.png[Мастер создания группы доступности]
+
...
+
.Мастер создания группы доступности
image::dev@platform:admin:always-on-sync-type.png[Мастер создания группы доступности]
+
. При переходе на страницу _Validation_ будет выполнена проверка создания группы доступности.
+
.Мастер создания группы доступности
image::dev@platform:admin:always-on-validation.png[Мастер создания группы доступности]
+
...
image::dev@platform:admin:always-on-state.png[Панель мониторинга группы доступности]
+
[NOTE]
====
...
image::dev@platform:admin:always-on-console.png[Настройки AlwaysOn в Консоли настройки {dv}]
+
