...
[NOTE]
====
Основной способ подключения сателлитной БД -- создать её через _{mc}_.
====

. Определите, поддерживает ли {mssql} с БД {dv} функцию секционированияfootnote:[См. описание установленной версии SQL Server.].
. Остановите сервер и сервисы {dv}.
. В SQL Server Management Studio перейдите в БД {dv} и выполните один из следующих скриптов:
+
* Если СУБД поддерживает секционирование -- скрипт из файла xref:platform:admin:attachment$ScriptCreateLogSatellit.sql[ScriptCreateLogSatellit.sql].
* Если СУБД не поддерживает секционирование -- скрипт из файла xref:platform:admin:attachment$ScriptCreateLogSatellitNoPartitions.sql[ScriptCreateLogSatellitNoPartitions.sql].
+
****
...
[%collapsible]
====
include::dev@platform:admin:partial$excerpts.adoc[tags=replaceName]
...
[%collapsible]
====
include::dev@platform:admin:partial$excerpts.adoc[tags=archiveName]
