...
Перенос базы данных между серверами {dv} может потребоваться, чтобы подключить БД к другому серверу {dv}, но в лицензии {dv} отсутствует опция _{dv} Concurrent Servers_, которая позволяет нескольким серверам {dv} работать с одной БД. При попытке подключить БД ко второму серверу {dv} возникнет предупреждение:
...
include::dev@platform:admin:partial$excerpts.adoc[tags=sat-move]
